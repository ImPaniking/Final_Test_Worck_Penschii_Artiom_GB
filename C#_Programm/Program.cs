...
var action = new Dictionary<int, string>()//очень хотел бы использовать это везде, но пока что могу только для красивого вывода условий
{
    { 1 , "="},
    { 2 , ">"},
    { 3 , ">="},
...
if ((condition > 0) && (condition < 7))
    conditionNumer = NumberInput($"число ( от 1 до 7 ) для условия поиска кол-во символов {action[condition]}");//пока не стал делать новый метод специально под эту проверку
...
if (condition == 7)// если условие 7 ( кол-во пустых эллементов ) нужно поменять схему вывода
{
